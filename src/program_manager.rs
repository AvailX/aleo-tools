...
use crate::{api::AleoAPIClient, encryptor::Encryptor};
...
impl<N: Network> ProgramManager<N> {
    ...
    pub(super) fn get_private_key(&self, password: Option<&str>) -> Result<PrivateKey<N>> {
        ...
        // TODO: Do we need this as ciphertext?
...
mod tests {
    ...
    use super::*;
    use crate::test_utils::{HELLO_PROGRAM, HELLO_PROGRAM_2, TESTNET_PRIVATE_KEY};
    ...
    fn test_program_management_methods() {
        ...
        let mut program_manager =
            ProgramManager::<Testnet3>::new(Some(private_key), None, None, None).unwrap();
    ...
    fn test_private_key_retrieval_from_ciphertext() {
        let private_key = PrivateKey::from_str(TESTNET_PRIVATE_KEY).unwrap();
        let private_key_ciphertext =
            Encryptor::<Testnet3>::encrypt_private_key_with_secret(&private_key, "password")
                .unwrap();
        let temp_dir = std::env::temp_dir();
