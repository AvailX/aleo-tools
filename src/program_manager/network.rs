...
impl<N: Network> ProgramManager<N> {
    ...
    pub fn api_client(&self) -> Result<&AleoAPIClient<N>> {
        self.api_client
            .as_ref()
            .ok_or_else(|| anyhow!("No API client found"))
    }
...
mod tests {
    ...
    use crate::{
        api::AleoAPIClient,
        test_utils::{random_program, GENERIC_PROGRAM_BODY, TESTNET_PRIVATE_KEY},
    };
    ...
    fn test_network_functionality_works_as_expected() {
        ...
        let random_program_state = program_manager
            .on_chain_program_state(&random_program)
            .unwrap();
        assert!(matches!(
            random_program_state,
