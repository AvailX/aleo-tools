...
use crate::errors::{AvailError, AvailErrorType};
...
impl<N: Network> ProgramManager<N> {
    ...
    pub fn find_program(&self, program_id: &ProgramID<N>) -> Result<Program<N>> {
        self.find_program_on_disk(program_id)
            .or_else(|_| self.find_program_on_chain(program_id))
    }

    /// Load a program from a local program directory
    pub fn find_program_on_disk(&self, program_id: &ProgramID<N>) -> Result<Program<N>> {
        let local_program_directory = self
            .local_program_directory
            .as_ref()
            .ok_or_else(|| anyhow!("Local program directory not set"))?;
        let imports_directory = local_program_directory.join("imports");
        // Ensure the directory path exists.
        ensure!(
            local_program_directory.exists(),
            "The program directory does not exist"
        );

        ensure!(
            !Program::is_reserved_keyword(program_id.name()),
            "Program name is invalid (reserved): {program_id}"
        );
        ...
        if manifest.program_id() == program_id {
            ...
            println!(
                "Attempting to load program {program_id:?} at {:?}",
                import_file.display()
            );
            let mut program_file = File::open(import_file)?;
            let mut program_string = String::new();
            match program_file.read_to_string(&mut program_string) {
                Ok(_) => {}
                Err(e) => {
                    AvailError::new(
                        AvailErrorType::Internal,
                        format!("Failed to read program file: {}", e),
                        "Failed to read program file".to_string(),
                    );
                }
            }
...
mod tests {
    ...
    fn test_file_loading_and_imports() {
        ...
        let imports = vec![
            ("credits.aleo", credits.as_str()),
            ("hello.aleo", HELLO_PROGRAM),
        ];
        let test_path =
            setup_directory("aleo_test_file_resolution", IMPORT_PROGRAM, imports).unwrap();
        ...
        let result = catch_unwind(|| {
            ...
            let bad_import_code = String::from("import ")
                .add(&random_program_id(16))
                .add(";")
                .add(IMPORT_PROGRAM);
    ...
    fn test_hybrid_program_and_import_loading() {
        ...
        let test_path =
            setup_directory("aleo_test_hybrid_resolution", IMPORT_PROGRAM, imports).unwrap();
        ...
        let result = catch_unwind(|| {
            ...
            assert_eq!(
                program_manager.find_program(credits_id).unwrap(),
                credits_program
            );
